...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
//PWM Capture for the Rising Edge
uint16_t pwm_rising;
...
INTERRUPT_HANDLER(TIM2_UPD_OVF_TRG_BRK_USART2_TX_IRQHandler,19)
{
    /* In order to detect unexpected events during development,
       it is recommended to set a breakpoint on the following instruction.
    */
	// This function is to generate a general system time for keeping track in functions
...
INTERRUPT_HANDLER(TIM1_CC_IRQHandler,24)
{
  ...
  //PWM rising and falling values automatically captured and CCR1 and CCR2. This function subtracts rising from falling and gives you the PWM width
  //As system clock is 1Mhz, the value is directly translatable to micro seconds. 
  if (TIM1_GetITStatus(TIM1_IT_CC2) != RESET){
	 pwm_width_us = TIM1_GetCapture2() - TIM1_GetCapture1();
	//Clear interrupt bit
	TIM1_ClearITPendingBit(TIM1_IT_CC2);
	//Set PWM flag to true
	pwm_sample_received = TRUE;
  }
